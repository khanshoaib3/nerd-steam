...
private data class RequirementItem(
...
private fun getRequirementItems(gameData: GameData): List<RequirementItem> {
    ...
    if (gameData.content?.data?.platforms?.windows == true
        ...
        requirementItems.add(
            RequirementItem(
                systemType = "Windows",
                minimumRequirements = pcRequirements.minimum?.replace("<br>", ""),
                recommendedRequirements = pcRequirements.recommended?.replace("<br>", "")
            )
        )
    }
    ...
    if (gameData.content?.data?.platforms?.linux == true
        ...
        requirementItems.add(
            RequirementItem(
                ...
                minimumRequirements = linuxRequirements.minimum?.replace("<br>", ""),
                recommendedRequirements = linuxRequirements.recommended?.replace("<br>", "")
            )
        )
    }
    ...
    if (gameData.content?.data?.platforms?.mac == true
        ...
        requirementItems.add(
            RequirementItem(
                systemType = "MacOS",
                minimumRequirements = macRequirements.minimum?.replace("<br>", ""),
                recommendedRequirements = macRequirements.recommended?.replace("<br>", "")
            )
        )
    }
        ...
        Column(
            ...
            requirementItems.forEach {
                ...
                Column(modifier = Modifier.padding(horizontal = dimensionResource(R.dimen.padding_small))) {
                    if (!it.minimumRequirements.isNullOrBlank()) {
                        Text(
                            AnnotatedString.fromHtml(it.minimumRequirements)
                        )
                    }

                    if (!it.recommendedRequirements.isNullOrBlank()) {
                        AnnotatedString.fromHtml(it.recommendedRequirements)
                    }
                }
            }
        }
...
@Composable
private fun SystemRequirementsPreview() {
    val gameRawData =
